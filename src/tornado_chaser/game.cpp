...
extern "C" void game_on_initialize(){
    ...
    scr->addItem(new ScreenItems::StaticText({100,100},"sample text",{255,255,255}));
    ScreenBuffer::load_from_mem("default",scr);
    ScreenManager::setScreen("default");
    scr = new Screen(false,true);
    scr->addItem(new ScreenItems::DynamicText({10,10},text));
    ScreenBuffer::load_from_mem("another",scr);
    *text = "FPS: ---";
};
extern "C" void game_on_frame(){
    //*text = "FPS: " + std::to_string(FPSCounter::fps);
    LiteGL::setParam(GL_Param::CullFace,true);
    LiteGL::setParam(GL_Param::Blend,true);
    ...
    if(Event::keyp(KBKey::P)){
        ScreenManager::setScreen("another");
    }
